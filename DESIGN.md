...
Split BullMQ worker out of the same Node process as the Fastify HTTP server: This keeps `docker compose up` to a single app container and makes local development friction-free, but it couples two jobs whose lifecycle and scaling profiles are different. In production the worker should be its own deployment.
