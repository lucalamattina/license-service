...
| `expires_at_in_past`         | 400  | `issue_license`                          | "The `expires_at` you provided is in the past. Re-read the human's request to confirm the intended duration before retrying. If the human said something relative like 'in 30 days', compute now + 30 days and try again. If they explicitly named a past date, ask them to clarify — they probably meant a future date but you should confirm before issuing." |
...
| `internal_error`             | 500  | Any tool                                 | "The backend returned an unexpected internal error. The MCP server did NOT retry (per the network-and-backend policy below: backend errors are surfaced on the first attempt, not retried, because the backend handled the request and chose to fail). Surface this to the human, include any reference id from the `details` field if present, and suggest they retry shortly or escalate to whoever operates the backend." |
...
Instead, each case is run **N=5 times per invocation** (configurable), and the runner reports a **pass rate** rather than a single pass/fail verdict. A case is considered **regressed** if its pass rate drops below a threshold the case itself declares (default 80%). This makes the difference between "always passes" (5/5), "small regression" (3/5 — investigate), and "broken" (0/5) explicit instead of collapsed into one bit.
...
Cost note: 12 cases × 5 samples = 60 agent runs per eval invocation. At Claude Sonnet's pricing, a small ($) budget. The harness records the cost per run and aborts if a configured cap is exceeded — useful when iterating on the suite locally so a typo doesn't accidentally run 600 cases.
